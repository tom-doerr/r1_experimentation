...
class Env1:
    ...
    def __call__(self, input_string: str) -> int:
        ...
        if len(input_string) >= self.char_count_penalty_start:
            ...
            return count - penalty  # Allow negative scores as per test requirements
        return count
...
class Env2:
    ...
    def __call__(self, input_string: str) -> int:
        ...
        if not isinstance(input_string, str):
            return 0
        return 1 if len(input_string) <= self.max_char_count else -1  # Return -1 for too long strings
