...
# don't mock
completion = litellm_completion('hi', model=MODEL) # the agent should respond
...
reward = test_env_1('aaa') # the agent should return 3
assert reward == 3

reward = test_env_1('aabbjadfa') # the agent should return 4
assert reward == 4
...
agent_assert = AgentAssert(model=MODEL) # the agent assert should be an instance of AgentAssert
...
return_val = agent_assert('twenty two has has the same meaning as 22') # the agent should return true
