...
print("last_completion:", last_completion) # type: ignore
...
assert agent.memory == parsed_data_2['memory']['replace']


agent_assert = AgentAssert(model=MODEL)
assert isinstance(agent_assert, AgentAssert)
...
assert {'ls', 'date'}.issubset(shell_code_executor.whitelisted_commands)
